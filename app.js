...
const animateValue = (element, start, end, type, duration = 500) => {
  ...
  const updateNumber = (currentTime) => {
    ...
    const current =
      type === "sats"
        ? Math.floor(start + range * progress)
        : (start + range * progress).toFixed(8);
...
const calculateSats = async () => {
  ...
  animateValue(
    satsOutput,
    parseInt(satsOutput.textContent.replace(/,/g, "") || "0"),
    sats,
    "sats"
  );
  animateValue(
    btcOutput,
    parseFloat(btcOutput.textContent || "0.0"),
    btcAmount,
    "btc"
  );
    ...
    function formatDecimal() {
      ...
      if (parts.length === 2) {
        ...
        const formattedValue =
          parts[0] + (parts[1] ? "." + parts[1].slice(0, decimalPlaces) : "");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if ("serviceWorker" in navigator) {
    window.addEventListener("load", () => {
      // Check if we're in development (localhost with any port)
      const isLocalhost =
        window.location.hostname === "localhost" ||
        window.location.hostname === "127.0.0.1" ||
        window.location.hostname === "";

      // Only register service worker in production or if not on localhost
      if (!isLocalhost || window.location.protocol === "https:") {
        ...
        console.log(
          "ServiceWorker registration skipped in development over HTTP"
        );
      }
    });
  }
